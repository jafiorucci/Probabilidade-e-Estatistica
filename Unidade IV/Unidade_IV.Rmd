...
**Def.:** O **valor esperado de uma função** $g(X)$ é dado
pela seguinte expressão: 
$$
...
Vamos assumir que $g$ é uma função monótona no suporte de $X$. \pause
\vspace{0.2cm}
...
$$
F_Y(y) = P(Y\le y) = P(X^2 \le y) = P(X \le \sqrt y) = F_X(\sqrt y) = \sqrt y
$$
para \(0 < y < 1\). \pause
...
Se $X \sim Uniforme(0, 1)$, qual a distribuição de $Y = -\log(X)$?

- Lembrete: Como $X \sim Uniforme(0, 1)$, então $F_X(x) = x,~\forall x \in (0, 1)$.
...
\end{aligned}
$$
Logo $Y \sim Exp(1)$, i.e., $Y$ tem distribuição Exponencial com parâmetro $\lambda=1$.
